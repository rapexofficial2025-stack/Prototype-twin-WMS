...
// Demo-mode occupancy, used only while no backend is configured (see isBackendConfigured in twin.api.ts).
const demoOccupancy = (column: number, level: number, depth: number) => ((column * 17 + level * 11 + depth * 7) % 100) < 70

export function Pallet({ column, side, level, depth, position }: PalletProps) {
  const { isLive, getLocation } = useTwinLookup()
  const live = isLive ? getLocation(side, column, level, depth) : undefined
  const occupied = isLive ? live?.status === 'occupied' : demoOccupancy(column, level, depth)
  ...
  return <group onClick={(event) => { event.stopPropagation(); window.dispatchEvent(new CustomEvent('frost:pallet-selected', { detail: { column, level: String.fromCharCode(65 + level), depth: depth + 1, status: occupied ? 'Occupied' : 'Available', itemName: live?.itemName ?? null, tagNo: live?.tagNo ?? null, customerName: live?.customerName ?? null, quantity: live?.quantity ?? null, batchNo: live?.batchNo ?? null, expirationDate: live?.expirationDate ?? null } })) }}>
