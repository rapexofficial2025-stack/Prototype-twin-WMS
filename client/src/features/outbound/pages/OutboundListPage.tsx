    ...
    <section>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Outbound shipment transactions.</p>
        </div>
        <Link to="/outbound/new" className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
              ...
              return (
                <Fragment key={doc.id}>
                  <tr onClick={() => setExpanded(isOpen ? null : doc.id)} className="cursor-pointer border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 font-medium text-slate-900">{doc.withdrawalNo}</td>
