    ...
    <div className="overflow-x-auto">
      <div className="flex gap-3 pb-2">
        {columns.map((column) => {
          ...
          return (
            <div key={column} className="w-[230px] shrink-0 overflow-hidden rounded-lg border border-slate-300 text-[11px]">
              <div className="grid grid-cols-[24px_repeat(4,1fr)] bg-slate-100 text-center font-semibold text-slate-600">
                <div className="border-r border-slate-300 py-1">L</div>
                {DEPTHS.map((d) => <div key={d} className="border-r border-slate-300 py-1 last:border-r-0">{d + 1}</div>)}
              </div>
              {rows.map(({ level, cells }) => (
                <div key={level} className="grid grid-cols-[24px_repeat(4,1fr)] border-t border-slate-200">
                  <div className="grid place-items-center border-r border-slate-200 bg-slate-50 font-semibold text-slate-500">{LEVEL_LETTERS[level]}</div>
                  {cells.map((cell, i) => (
                    <div key={i} className={`min-h-11 border-r border-slate-200 p-1 text-center last:border-r-0 ${cell.occupied ? 'bg-blue-50' : 'bg-white text-slate-300'}`}>
                      {cell.occupied ? <>
                        <div className="truncate font-medium text-slate-900">{cell.itemName}</div>
                        <div className="truncate text-slate-500">{cell.batch}</div>
                        <div className="font-semibold text-blue-700">{cell.quantity}</div>
                      </> : '—'}
                    </div>
                  ))}
                </div>
              ))}
              <div className="border-t border-slate-300 bg-slate-800 py-1.5 text-center font-semibold text-white">
                COLUMN {column} <span className="ml-1 text-blue-300">{total}</span>
              </div>
            </div>
          )
        })}
