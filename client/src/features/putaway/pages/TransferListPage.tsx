  ...
  return (
    <section>
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500">Internal warehouse movement between locations.</p>
      </div>
        ...
        {transferDocs.map((doc) => (
          <Card key={doc.id} className="p-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="font-medium text-slate-900">{doc.transferNo}</p>
                <p className="text-xs text-slate-500">{doc.customerName} · {doc.date}</p>
              </div>
